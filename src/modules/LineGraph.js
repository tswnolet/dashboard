...
const LineGraph = ({ data, title, secondData = null, yAxisLabel = "count" }) => {
    ...
    useEffect(() => {
        ...
        dateKeys.forEach(date => {
            ...
            formattedTotals[formattedDate] += parseFloat(data[date][0]) || 0;
            if (secondData) {
                formattedSecondTotals[formattedDate] += parseFloat(data[date][1]) || 0;
            }
        });

        const years = Object.keys(formattedTotals).sort();
        const values = years.map(year => formattedTotals[year] || 0);
        const secondValues = Object.keys(formattedSecondTotals).length > 0
            ? years.map(year => formattedSecondTotals[year] || 0)
