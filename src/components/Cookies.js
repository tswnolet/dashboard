...
const CloseSvg = ({ onClick }) => (
  <div id='closeSvg'>
    <svg viewBox="0 0 329.26933 329" xmlns="http://www.w3.org/2000/svg" id="fi_1828778" onClick={onClick}>
...
export default function Cookies() {
  ...
  useEffect(() => {
    setCookieAcceptance(getCookie('cookie-acceptance'));
  }, []);

  const handleAccept = () => {
    setCookie('cookie-acceptance', true, 365);
    setCookieAcceptance(true);
  };

  const handleDecline = () => {
    setCookie('cookie-acceptance', false, 365);
    ...
    (cookieAcceptance === null || cookieAcceptance === '') && (
      <div id='cookies'>
        {!readPolicy ? (
          <>
            <CookieSvg />
            <h1 className='cookieHeader'>We use cookies.</h1>
